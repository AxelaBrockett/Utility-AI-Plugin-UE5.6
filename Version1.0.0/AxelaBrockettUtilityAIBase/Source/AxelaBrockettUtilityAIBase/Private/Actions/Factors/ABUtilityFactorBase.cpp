...
const float UABUtilityFactorBase::GetFactorScore(const UABUtilityComponent& OwnerComp, const float NumberOfFactors) const
{
...
) const
{
	...
	DebuggerCategory->AddTextLine(
		FString::Printf(TEXT("                {red}Inverse Score: %s / Use Consideration Factor: %s / Current Score: %f"),
			...
			IsUsingConsiderationFactor() ? TEXT("true") : TEXT("false"),
			GetFactorScore(OwnerComp, NumberOfFactors)
		)
	);
}
#endif //WITH_GAMEPLAY_DEBUGGER_MENU