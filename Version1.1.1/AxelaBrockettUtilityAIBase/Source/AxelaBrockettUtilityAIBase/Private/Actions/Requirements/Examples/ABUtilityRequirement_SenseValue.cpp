...
const bool UABUtilityRequirement_SenseValue::CheckRequirement(const UABUtilityComponent& OwnerComp) const
{
	...
	GoalValue.BindData(AIController->GetPawn(), INDEX_NONE);
	...
	if (!IsValid(PerceptionComponent))
	{
		ABUtility::Helpers::UtilityVLOG(
			ELogVerbosity::Error,
			FString::Printf(TEXT("%s - Could not get valid Perception Component from AI Controller [ %s ]."),
				ANSI_TO_TCHAR(__FUNCTION__),
	...
	if (!SenseID.IsValid())
	{
		ABUtility::Helpers::UtilityVLOG(
			...
			FString::Printf(TEXT("%s - Could not get valid sense ID. Result will be generic."),
				ANSI_TO_TCHAR(__FUNCTION__)
			),
			AIController
		);
	...
	const bool bComparison = ABUtility::Helpers::CompareValues(HighestStrengthValue, GVValue, ValueComparison);
	return bComparison;
}
