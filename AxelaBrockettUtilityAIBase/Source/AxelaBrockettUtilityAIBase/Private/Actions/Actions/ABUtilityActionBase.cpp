...
const float UABUtilityActionBase::GetActionScore(const UABUtilityComponent& OwnerComp) const
{
...
#if WITH_GAMEPLAY_DEBUGGER_MENU
void UABUtilityActionBase::DescribeSelfToGameplayDebugger(const UABUtilityComponent& OwnerComp, FGameplayDebuggerCategory* DebuggerCategory) const
{
	...
	if (IsValid(AIController))
	{
		UtilityWeight.BindData(AIController, INDEX_NONE);
		OnSuccessCooldown.BindData(AIController, INDEX_NONE);
	}
	...
	{
		Factor->DescribeSelfToGameplayDebugger(OwnerComp, NumFactors, DebuggerCategory);
	}
}
#endif //WITH_GAMEPLAY_DEBUGGER_MENU
