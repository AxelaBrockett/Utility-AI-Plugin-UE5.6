...
void UABUtilityAction_MoveTo::PauseAction(UABUtilityComponent* OwnerComp, const FString& Reason)
{
	...
	AbortMoveOnPause.BindData(AIController, INDEX_NONE);
...
#if WITH_GAMEPLAY_DEBUGGER_MENU
void UABUtilityAction_MoveTo::DescribeSelfToGameplayDebugger(const UABUtilityComponent& OwnerComp, FGameplayDebuggerCategory* DebuggerCategory) const
{
	...
	if (IsValid(AIController))
	{
		AllowPartialPath.BindData(AIController, INDEX_NONE);
		AllowStrafe.BindData(AIController, INDEX_NONE);
		ReachTestIncludesAgentRadius.BindData(AIController, INDEX_NONE);
		ReachTestIncludesGoalRadius.BindData(AIController, INDEX_NONE);
		RequireNavigableEndLocation.BindData(AIController, INDEX_NONE);
		ProjectGoalLocation.BindData(AIController, INDEX_NONE);
		UsePathFinding.BindData(AIController, INDEX_NONE);
		StartFromPreviousPath.BindData(AIController, INDEX_NONE);
		TrackMovingActor.BindData(AIController, INDEX_NONE);
		AbortMoveOnPause.BindData(AIController, INDEX_NONE);
		AcceptableRadius.BindData(AIController, INDEX_NONE);
		ActorTrackingGoalMovementTolerance.BindData(AIController, INDEX_NONE);
	}
